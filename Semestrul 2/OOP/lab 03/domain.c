...
cheltuiala copiaza_cheltuiala_for_delete(cheltuiala new_chelt, cheltuiala chelt) {
    new_chelt.id_cheltuiala = chelt.id_cheltuiala - 1;
    new_chelt.nr_apartament = chelt.nr_apartament;
    new_chelt.suma_cheltuiala = chelt.suma_cheltuiala;
    strcpy(new_chelt.tip_cheltuiala, chelt.tip_cheltuiala);
...
void run_domain_tests() {
    ...
    test_copiaza_cheltuiala_for_delete();
}
...
void test_copiaza_cheltuiala_for_delete(){
    char *tip = (char*)malloc(sizeof(char) * 32);
    strcpy(tip, "internet");
    cheltuiala chelt = create_cheltuiala(0, 12, 250, tip);
    ...
    strcpy(tip2, "apa");
    cheltuiala chelt2 = create_cheltuiala(1, 24, 123, tip2);

    chelt = copiaza_cheltuiala_for_delete(chelt, chelt2);

    assert(get_id_cheltuiala(chelt) == 0);
    assert(get_numar_apartament(chelt) == 24);
    assert(get_suma_cheltuiala(chelt) == 123);

    char* tip_chelt = get_tip_cheltuiala(chelt);
    assert(strcmp(tip_chelt, "apa") == 0);
    free(tip_chelt);
