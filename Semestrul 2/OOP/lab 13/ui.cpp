...
const void ui_operations::print_meds(AbstractRepo& storage, const string& header) const{
    std::cout << header;
...
const int ui_operations::read_id_with_string(AbstractRepo &storage, const string &msg_out, const string &msg_err) {
    while(true) {
        std::cout << msg_out;
...
const void ui_operations::filterPret(AbstractRepo &storage, int pret, char op) const {
...
const void ui_operations::filterSubstActiva(AbstractRepo &storage, const string &subst) const {
    vector<Medicament>& vec_from = storage.get_all();
    vector<Medicament> vector_filtrat;
    vector_filtrat.clear();
...
const void ui_operations::genericSort(AbstractRepo &storage,
...
const void ui_operations::print_raport(std::unordered_map<string, MedicamentDTO> &raport) {
    ...
    for(auto & it : raport) {
        std::cout << "Substanta activa: " << std::setw(15) << it.first << ", Numar medicamente: " << it.second.nr_meds << '\n';
    }
}
