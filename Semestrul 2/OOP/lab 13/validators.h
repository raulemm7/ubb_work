...
public:
    ...
    friend std::ostream& operator<<(std::ostream& out,const BadLuckException& ex){
        out << ex.msg << '\n';
        return out;
    };
};
...
public:
     ...
     *          0 => altfel
     */
    const bool valideaza_med(const AbstractRepo& storage, const Medicament& medicament);

    static const int valideaza_med_dupa_denumire(AbstractRepo& storage, const string& denumire);
