...
MyVector testCopyIterate(MyVector v) {
    double totalPrice = 0;
    for (int i = 0; i < v.get_last_id_vd(); i++) {
        Pet el = v.get_element(i);
...
void testCreateCopyAssign() {
    ...
    addPets(v, 100);
    assert(v.get_last_id_vd() == 100);

    MyVector v2{ v };//constructor de copiere
    assert(v2.get_last_id_vd() == 100);

    MyVector v3;//default constructor
    v3 = v;//operator=   assignment
    assert(v3.get_last_id_vd() == 100);

    auto v4 = testCopyIterate(v3);
    assert(v4.get_last_id_vd() == 101);*/
    MyVector v;//default constructor
    addPets(v, 100);
    assert(v.get_last_id_vd() == 100);
    assert(v.get_element(50).getType() == "50_type");

    MyVector v2{ v };//constructor de copiere
    assert(v2.get_last_id_vd() == 100);
    assert(v2.get_element(50).getType() == "50_type");

    MyVector v3;//default constructor
    v3 = v;//operator=   assignment
    assert(v3.get_last_id_vd() == 100);
    assert(v3.get_element(50).getType() == "50_type");

    auto v4 = testCopyIterate(v3);
    assert(v4.get_last_id_vd() == 101);
    assert(v4.get_element(50).getType() == "50_type");
}
