...
void MedicamentGUI::init_gui() {
    ...
    // left layout
    this->tabel_view->setModel(this->model);
    // setez ca elementele pe care le poti selecta din tabel sa fie randurile
    this->tabel_view->setSelectionBehavior(QAbstractItemView::SelectRows);
    this->leftLayout->addWidget(this->tabel_view);
...
void MedicamentGUI::connect_buttons() {
    QObject::connect(this->listaMedicamente, &QListWidget::itemSelectionChanged, [&](){
       ...
       if(selected.isEmpty()){
           ...
           this->denumire->setText(QString::fromStdString(med.get_denumire()));
    ...
    // functionalitate table_model
    // atunci cand un rand este selectat, se actualizeaza informatiile unui medicament
    // in label
    QObject::connect(this->tabel_view, &QTableView::clicked, this, [&](){
        auto selected = this->tabel_view->currentIndex();
...
void MedicamentGUI::gui_adauga_med() {
    ...
    this->load_data_in_list(this->storage.get_all());
    this->load_data_in_table(this->storage.get_all());
    this->load_data_in_table_model(this->storage.get_all());
...
void MedicamentGUI::gui_sort_denumrie() {
    ...
    this->load_data_in_table_model(meds);
}
...
void MedicamentGUI::genereaza_random_gui() {
    ...
    string m = "Medicamente generate cu succes!";
    MedicamentGUI::message_box(m);
}

void MedicamentGUI::load_data_in_table_model(const vector<Medicament> &meds) {
    this->model->setList(meds);
}
