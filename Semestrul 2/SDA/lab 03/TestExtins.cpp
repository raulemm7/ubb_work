...
void testCreeaza() {
    ...
    cout << "testCreeaza trecut cu succes!\n";
}

void testAdauga() {
    cout << "rulare testAdauga...\n";
...
void testSterge() {
    ...
    cout << "testSterge finalizat cu succes!\n";
}
...
void testIterator() { // nu stim reprezentarea Colectiei, putem testa doar anumite lucruri generale, nu stim in ce ordine vor fi afisate elementele.
    ...
    cout << "testIterator finalizat cu succes!\n";
}

void testQuantity() {//scopul e sa adaugam multe date
    cout << "rulare testQuantity...\n";
    long int iteratie = 0;
    ...
    for (int i = 10; i >= 1; i--) {
        for (int j = -30000; j < 30000; j = j + i) {
            c.adauga(j);
            cout << "iteratia: " << iteratie << ", inca rulez (part1)testQuantity...\n";
            iteratie++;
        }
    }
    assert(c.dim() == 175739);
    /*c.afiseaza_elemente();
    cout << c.dim() << " " << c.nr_elemente_distincte() << '\n';
    return;*/
    ...
    for (int i = 0; i < c.dim(); i++) {
        ic.urmator();
        cout << "iteratia: " << iteratie << ", inca rulez (part2)testQuantity...\n";
        iteratie++;
    }
    ic.prim();
    iteratie = 0;
    while (ic.valid()) { //fiecare element returnat de iterator trebuie sa fie in colectie si nrAparitii trebuie sa fie mai mare ca 0
        ...
        ic.urmator();
        cout << "iteratia: " << iteratie << ", inca rulez (part3)testQuantity...\n";
        iteratie++;
    }
    assert(ic.valid() == false);
    iteratie = 0;
    for (int i = 0; i < 10; i++) { //stergem multe elemente existente si inexistente
        for (int j = 40000; j >= -40000; j--) {
            c.sterge(j);
            cout << "iteratia: " << iteratie << ", inca rulez (part4)testQuantity...\n";
            iteratie++;
        }
    }
    assert(c.dim() == 0);

    cout << "testQuantity finalizat cu succes!\n";
}
