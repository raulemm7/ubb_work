...
public class FriendshipValidator implements Validator<Friendship> {
    ...
    public void validate(Collection<Friendship> allEntities, Friendship friendship) {
        for(Friendship entity : allEntities) {
            if(entity.getFirstUser().equals(friendship.getFirstUser())
                    && entity.getSecondUser().equals(friendship.getSecondUser())) {
                throw new ValidationException("Aceasta prietenie exista deja!");
            }

            if(entity.getFirstUser().equals(friendship.getSecondUser())
                    && entity.getSecondUser().equals(friendship.getFirstUser())) {
                throw new ValidationException("Aceasta prietenie exista deja!");
            }
        }
    }
